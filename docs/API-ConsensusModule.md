...
| `TCPRPCTimeout`             | 200 мс   | Тайм-аут TCP RPC в промышленном транспорте: окно одного RPC-обмена запрос/ответ; производные — окно проверки кворума = 2 × 200 = 400 мс, потолок пульса = ⌊400/4⌋ = 100 мс. |
| `ConnectionTCPRPCTimeout`   | 165 мс   | Дедлайн установки TCP-соединения (net.DialTimeout); историческое значение RPC-тайм-аута.                                                                                    |
| `InstallSnapshotTimeout`    | 310 мс   | Опорная длительность для расчёта предельного времени передачи снимка. Итоговое время вычисляется как: `310 мс × ⌊DataSize / 256 KiB⌋` на стороне отправителя.               |
...
| `DefaultReelectionTimeout`  | 430 мс   | Опорная длительность: на её основе формируется случайный тайм‑аут выборов в диапазоне [430; 860) мс.                                                                        |
