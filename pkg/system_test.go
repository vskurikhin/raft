...
func TestBasicAppendSameClient(t *testing.T) {
	...
	// Добавление к существующему ключу.
	...
	// Добавление к несуществующему ключу.
...
func TestAppendLinearizableAfterDelay(t *testing.T) {
	...
	// Последовательность put+append; проверяем, что получаем ожидаемый результат.
	h.CheckPut(c1, "foo", "bar")
	h.CheckAppend(c1, "foo", "baz")
	h.CheckGet(c1, "foo", "barbaz")

	// Попросить сервис задержать ответ на следующий запрос и отправить Append.
	// Клиент повторит этот запрос, поэтому система должна корректно обработать
	// повторную отправку. Клиент получит сообщение о дублирующемся запросе,
	// но операция Append должна успешно выполниться.
...
func TestAppendLinearizableAfterCrash(t *testing.T) {
	...
	// Задержать ответ от лидера, а затем завершить его работу.
	// После выбора нового лидера ожидается, что операция Append будет
