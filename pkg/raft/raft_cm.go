...
type ConsensusModule struct {
	...
	// logNeedsPersist — флаг, указывающий, что журнал изменился
	// и требует пересохранения в persistToStorage.
	// Позволяет избежать дорогого gob.Encode(cm.log) при каждом
	// heartbeat или RPC, когда лог не менялся.
...
func (cm *ConsensusModule) Stop() {
	...
	cm.traceLogf(1, "CM.Stop called / becomes Dead")
	close(cm.shutdownCh)
}
...
func (cm *ConsensusModule) becomeFollower(term int) {
	...
	cm.traceLogf(1, "becomes Follower with term=%d; len(log)=%v", term, len(cm.log))
...
func (cm *ConsensusModule) handleFsmSnapshot(req *reqSnapshotFuture) {
	...
	defer func() {
		...
		cm.traceLogf(0, "FSM snapshot handling took %v", elapsed)
	}()
	if cm.lastApplied < 0 {
...
func (cm *ConsensusModule) handleInstallSnapshot(rpc RPC, req *InstallSnapshotRequest) {
	...
	defer func() {
		elapsed := time.Since(startNow)
		cm.traceLogf(0, "InstallSnapshot elapsed %v", elapsed)
	}()
...
func (cm *ConsensusModule) processLogs(commitIndex int) {
	...
	defer func() {
		elapsed := time.Since(startNow)
		cm.traceLogf(0, "Logs processing elapsed %v", elapsed)
	}()
	cm.mu.Lock()
...
func (cm *ConsensusModule) runFSM() {
	...
	defer func() {
		elapsed := time.Since(startNow)
		cm.traceLogf(0, "FSM timer elapsed %v", elapsed)
	}()
...
func (cm *ConsensusModule) runLeaderLoop() {
	...
	defer func() {
		...
		// Сбросить inflightAE для всех peer при выходе из leader loop.
...
func (cm *ConsensusModule) runPreCandidate() {
	...
	cm.traceLogf(1, "becomes PreCandidate; term=%d, len(log)=%d", cm.currentTerm, len(cm.log))
...
func (cm *ConsensusModule) sendBatch(start, end int) {
	...
	defer func() {
		elapsed := time.Since(startNow)
		cm.traceLogf(0, "sendBatch: elapsed %s", elapsed)
	}()
	cm.mu.Lock()
...
func (cm *ConsensusModule) startElection() {
	...
	defer func() {
		...
		cm.traceLogf(0, "startElection: elapsed %s", elapsed)
	}()
	cm.state = Candidate
	cm.currentTerm += 1
	savedCurrentTerm := cm.currentTerm
	...
	cm.votedFor = cm.id
	cm.persistToStorage()
	cm.traceLogf(1, "becomes Candidate (currentTerm=%d); len(log)=%v", savedCurrentTerm, len(cm.log))
...
func (cm *ConsensusModule) startLeader() {
	...
	for _, s := range cfg.ConfigServers {
		...
		if cm.inflightAE[peerID] == nil {
			cm.inflightAE[peerID] = new(atomic.Bool)
		}
		cm.inflightAE[peerID].Store(false)
	}
	cm.traceLogf(
		1, "becomes Leader; term=%d, nextIndex=%v, matchIndex=%v; len(log)=%d",
...
func (cm *ConsensusModule) startStopReplication(cfg Configuration) {
	...
	for _, s := range cfg.ConfigServers {
		...
		if _, ok := cm.inflightAE[peerID]; !ok {
			cm.inflightAE[peerID] = new(atomic.Bool)
		}
		cm.inflightAE[peerID].Store(false)
	}
...
func (cm *ConsensusModule) takeSnapshot() error {
	...
	defer func() {
		elapsed := time.Since(start)
		cm.traceLogf(0, "takeSnapshot elapsed %s", elapsed)
	}()
