...
// сколько бы повторных heartbeat-ack'ов ни прислал тот же follower
...
// Окно ожидания включает несколько heartbeat-интервалов (SA-029:
...
// TestVerifyLeader_NonvoterAckDoesNotVote — nonvoter-негатив (SA-018):
// после DemoteVoter живы только лидер и nonvoter с непрерывными
// ack'ами — подтверждение НЕ наступает: ack nonvoter'а голосом не
// является, порог 2 голосов (3 voters) собирается только из voter'ов.
//
...
func TestVerifyLeader_NonvoterAckDoesNotVote(t *testing.T) {
	...
	// Демотировать одного follower'а в nonvoter и дождаться коммита
	...
	// Отключить остальных voter-follower'ов: живы лидер + nonvoter.
	for i := 0; i < h.n; i++ {
		if i == lid || i == demoteID {
			continue
		}
	...
	// Негативное окно с запасом: ack'и nonvoter'а не голосуют.
	select {
	case err := <-future.ErrorCh():
...
// повторные голоса одного peerID не накапливаются (SA-004). Прямая
// проверка счётчика: два голоса одного узла дают votes=1, голос
// другого узла — votes=2.
func TestVerifyFuture_VoteDeduplication(t *testing.T) {
	...
	vf.vote(1, true)
	vf.vote(1, true) // повторный heartbeat-ack того же узла — no-op
	if vf.votes != 1 {
...
// TestVerifyLeader_EpochFilter — белый короб на фильтр по раунду
// (SA-016): голос засчитывается только от AE, отправленного не раньше
// постановки запроса (vf.epoch <= dispatchEpoch отправившей горутины).
//
// Один и тот же success-ответ AE: при dispatchEpoch=1 голос для
// запроса epoch=2 не засчитывается; при dispatchEpoch=2 — засчитывается.
