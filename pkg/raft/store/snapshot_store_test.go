...
func TestInmemSnapshot_CreateAndList(t *testing.T) {
	...
	// List должен вернуть один снимок с правильными метаданными.
	...
	sink2, err := store.Create(20, 4, raft.Configuration{}, 10)
	if err != nil {
		t.Fatalf("Create failed: %v", err)
	}
	_ = sink2.Close()
...
func TestInmemSnapshot_Open(t *testing.T) {
	...
	store := NewInmemSnapshot()
	sink, err := store.Create(10, 3, raft.Configuration{}, 0)
	if err != nil {
		t.Fatalf("Create failed: %v", err)
	}
	...
	// Открытие несуществующего снимка — ошибка.
...
func TestInmemSnapshotSink_WriteCloseCancel(t *testing.T) {
	...
	store := NewInmemSnapshot()
	sink, err := store.Create(5, 2, raft.Configuration{}, 0)
	if err != nil {
		t.Fatalf("Create failed: %v", err)
	}
...
func TestInmemSnapshot_ListDoesNotMutate(t *testing.T) {
	...
	store := NewInmemSnapshot()
	sink, err := store.Create(10, 3, raft.Configuration{}, 5)
	if err != nil {
		t.Fatalf("Create failed: %v", err)
	}
	_ = sink.Close()
...
func TestInmemStore_CancelKeepsPrevious(t *testing.T) {
	...
	store := NewInmemSnapshot()
	sinkA, err := store.Create(10, 1, raft.Configuration{}, 10)
	if err != nil {
	...
	sinkB, err := store.Create(11, 1, raft.Configuration{}, 11)
	if err != nil {
