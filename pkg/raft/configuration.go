...
func nextConfiguration(current Configuration, currentIndex int, req configurationChangeRequest) (Configuration, error) {
	...
	case RemoveServer:
		...
		// self-removal разрешён, но последний voter не может быть удалён
		if err := checkConfiguration(Configuration{ConfigServers: servers}); err != nil {
			return Configuration{}, err
		}
		_ = removed
...
// voterIDs возвращает список ID voter'ов из конфигурации.
...
// hasVote проверяет, является ли сервер с данным ID voter'ом в конфигурации.
