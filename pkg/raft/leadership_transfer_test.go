...
//  2. Отправляем команды 5 и 6, убеждаемся что они закоммичены.
...
func TestLeadershipTransfer_Basic(t *testing.T) {
	...
	// Отправляем команды и дожидаемся коммита.
	...
	// Проверяем, что старые команды закоммичены.
...
// передача лидерства, запущенная во время первой, возвращает
// ErrLeadershipTransferInProgress.
//
// Сценарий:
//  1. Запускаем первую передачу (LeadershipTransfer(follower1)).
//  2. Немедленно запускаем вторую передачу (LeadershipTransfer(follower2)).
...
func TestLeadershipTransfer_Concurrent_SecondFails(t *testing.T) {
	...
	// Выбираем двух follower.
	follower1 := (origLeaderID + 1) % 3
	follower2 := (origLeaderID + 2) % 3

	// Запускаем первую передачу в горутине, чтобы не блокировать тест.
	firstCh := make(chan error, 1)
	go func() {
		future := h.LeadershipTransfer(origLeaderID, ServerID(follower1))
	...
	// уже завершилась — `future.respond(nil)` теперь вызывается в stepDown).
	secondFuture := h.LeadershipTransfer(origLeaderID, ServerID(follower2))
	if err := secondFuture.Error(); err != ErrLeadershipTransferInProgress && err != ErrNotLeader {
		t.Fatalf("second transfer: got %v, want ErrLeadershipTransferInProgress or ErrNotLeader", err)
	}

	// Дожидаемся завершения первой передачи.
	select {
	case err := <-firstCh:
		if err != nil {
			t.Logf("first transfer completed with: %v", err)
		}
	case <-time.After(10 * time.Second):
		t.Fatal("first transfer timed out")
	}
}
