...
// --- Тесты идемпотентности InstallSnapshot (TASK-003, ADR-P07-003). ---

// countingSnapshotStore — обёртка над InmemSnapshotStore, считающая
// вызовы Create: признак «снапшот действительно применялся».
...
// снапшотом (lastSnapshotIndex=10, term 2, пустой журнал) для прямого
...
// installSnapshotRequestData — gob-данные снапшота для snapshotTestFSM.
...
// TestInstallSnapshot_RepeatedIsNoOp — TASK-003 тест 1: два подряд
...
func TestInstallSnapshot_RepeatedIsNoOp(t *testing.T) {
	...
	// Повторная установка того же снапшота — no-op.
	reply = driveInstallSnapshotRPC(t, cm, req, data)
	if !reply.Success {
...
// TestInstallSnapshot_StaleDoesNotRollBack — TASK-003 тест 2: InstallSnapshot
...
// TestInstallSnapshot_NoOpDrainsBody — TASK-003 тест 3 (часть 1): при no-op
...
// TestInstallSnapshot_NoOpKeepsTCPConnectionUsable — TASK-003 тест 3
...
func TestInstallSnapshot_NoOpKeepsTCPConnectionUsable(t *testing.T) {
	...
	// Узел уже владеет снапшотом на индексе 10.
	cm.mu.Lock()
	cm.cmState.lastSnapshotIndex = 10
	cm.cmState.lastSnapshotTerm = 2
	cm.cmState.lastLogIndex = 10
