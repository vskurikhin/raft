...
const (
	...
	// хранит снапшоты. Не конфликтует с FileStorage.loadAll: тот читает
	// только *.dat-файлы в корне каталога.
	snapshotsSubdir = "snapshots"

	// metaFileName — имя файла метаданных внутри директории снапшота.
	metaFileName = "meta.dat"

	// stateFileName — имя файла данных FSM внутри директории снапшота.
	stateFileName = "state.bin"

	// tmpSuffix — суффикс временной директории незавершённого снапшота.
	tmpSuffix = ".tmp"
)

// fileSnapshotMeta — метаданные снапшота на диске: SnapshotMeta
...
// FileSnapshotStore — файловая durable-реализация SnapshotStore.
// Снапшоты хранятся в <base>/snapshots/<term>-<index>-<msec>/;
...
// snapshots каталога base. retain >= 1 — сколько снапшотов сохранять;
// при создании нового снапшота более старые удаляются.
...
// testPermissions проверяет, что в каталог снапшотов можно писать.
...
// snapshotName генерирует имя директории снапшота: <term>-<index>-<msec>.
func snapshotName(term, index int, msec int64) string {
	return fmt.Sprintf("%d-%d-%d", term, index, msec)
}

// Create создаёт новый снапшот с указанными метаданными. Снапшот не виден
...
// List возвращает метаданные всех доступных снапшотов, упорядоченные
// от новых к старым (по Term, затем Index, затем ID). Повреждённые
// снапшоты пропускаются с логированием.
...
// Open открывает снапшот по ID, проверяет CRC32 данных и возвращает
...
// getSnapshots возвращает все известные снапшоты, отсортированные
// от новых к старым.
...
// readMeta читает и декодирует meta.dat снапшота id.
...
// removeOldSnapshots удаляет снапшоты сверх retain (самые старые).
...
// Close закрывает файл состояния снапшота.
func (b *bufferedReadCloser) Close() error {
	return b.fh.Close()
}

// FileSnapshotSink — реализация SnapshotSink, пишущая данные FSM
// во временную директорию снапшота.
...
// Close завершает запись снапшота: flush, fsync и закрытие state.bin,
// запись meta.dat с CRC32, атомарный rename временной директории в
// финальное имя, fsync родительской директории и удаление снапшотов
...
func (s *FileSnapshotSink) Close() error {
	...
	// Удаление снапшотов сверх retain — best-effort: ошибка не ломает
	// успешный Close. Удаление директории, из которой другой горутиной
	// открыт state.bin на чтение (leaderSendSnapshot), безопасно по
	// POSIX-семантике unlink; не-POSIX файловые системы не поддерживаются.
...
// Cancel отменяет запись снапшота: удаляет временную директорию целиком,
// не затрагивая ранее созданные снапшоты. Идемпотентен.
...
// ID возвращает идентификатор снапшота.
