...
//
// Контракт владения переданной записью (INV-1..INV-3): ConsensusModule
// не мутирует запись журнала и её payload после коммита — FSM получает
// собственную копию структуры LogEntry. При условии, что вызывающий Apply
// не мутирует переданную команду после передачи, payload иммутабелен и его
...
//
// Контракт жизненного цикла (ADR-003 п.6): Apply обязан завершаться —
...
// не мутирует команду после передачи).
