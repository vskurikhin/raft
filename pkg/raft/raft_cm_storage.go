...
//
// Порядок записи ключей важен для крэш-безопасности (TASK-008):
// currentTerm, votedFor, lastSnapshotIndex, lastSnapshotTerm, затем log.
// Снапшот-ключи пишутся ДО усечённого лога: окно «лог усечён, а
...
// checkSnapshotKeysConsistency проверяет, что для компактированного лога
// (первая запись с Index > 0) в storage присутствуют ключи
// lastSnapshotIndex/lastSnapshotTerm. Их отсутствие означало бы тихую
// потерю метаданных снапшота при усечённом durable-логе (TASK-008).
...
// restoreFromSnapshotStore восстанавливает FSM из последнего снапшота
// SnapshotStore и синхронизирует lastApplied/commitIndex/lastLog* с
// метаданными снапшота. Должен вызываться в конструкторе до запуска
...
func (cm *ConsensusModule) restoreFromSnapshotStore() error {
	...
	if len(snapshots) > 0 {
		...
		// Восстановление конфигурации кластера из метаданных снапшота
		// (TASK-010): снапшот берёт configurations.committed в takeSnapshot,
		// поэтому конфигурация снапшота закоммичена по определению. Применяется,
		// только если новее конфигурации, восстановленной из суффикса лога;
		// пустая конфигурация (легаси/тестовые снапшоты) игнорируется.
		...
		if meta.Index != cm.cmState.lastSnapshotIndex {
			// Окно крэша между Close снапшота и persistToStorage
			// (FINDING-007): снапшот в durable-сторе новее диска —
		...
		if cm.cmState.lastLogIndex < meta.Index {
			...
			// lastLogIndex/lastLogTerm со снапшотом — симметрично
...
// относительно снапшота. Выделена в отдельную функцию для тестирования
...
func (cm *ConsensusModule) checkSnapshotLogContinuity() error {
	...
	if cm.cmState.lastLogIndex < cm.cmState.lastSnapshotIndex {
		...
		// lastLogIndex со снапшотом — defense-in-depth против регрессий.
		return fmt.Errorf(
