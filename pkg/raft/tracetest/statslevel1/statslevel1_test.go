// Package statslevel1_test — матрица периодической статистики на уровне
// трассировки 1: один уровень задаётся один раз на процесс до создания
// первого CM. Обе настройки вывода (stats-output true/false) проверяются
// последовательно в одном процессе без изменения уровня.
package statslevel1_test
...
func captureStdout(t *testing.T) string {
	t.Helper()
	f, err := os.CreateTemp("", "raft-statslevel1-*.log")
	if err != nil {
		t.Fatal(err)
	}
	path := f.Name()
