...
// зависание (deadlock), которое по AC обязано быть FAIL (R-N11:
...
// (ADR-003 п.5, SA-013): любой возврат из Stop(), включая повторный и
// параллельный, гарантирует завершение всех горутин CM.
...
// deadlock по NEW-01/NEW-02 проявляется зависанием, а не паникой).
...
// TestJoinFiniteWhenReceiverStopped — порядок Stop/Close (ADR-003 п.2,
// SA-004): CM-получатель остановлен, его транспорт открыт; отправитель
...
// TestCrashPeerQuiesce — CrashPeer + in-flight commit (ADR-003 п.7,
// SA-005): после CrashPeer h.commits[id] пуст и остаётся пустым —
...
func TestCrashPeerQuiesce(t *testing.T) {
	...
	// Подать команды и немедленно уронить follower — коммит может быть
