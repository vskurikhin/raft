...
// writeSnapshotData записывает снапшот с gob-кодированным map в
...
// waitForSnapshotAndCompaction ждёт, пока узел создаст снапшот и
// фактически компактирует лог.
...
// waitForSnapshotQuiescence ждёт, пока цикл снапшотов узла гарантированно
// завершит все записи: persisted lastSnapshotIndex совпадает с in-memory
// значением и порог нового снапшота не достигнут. Необходимо перед
...
// пересечься с записью нового инстанса (общий <key>.dat.tmp).
...
// FileStorage + FileSnapshotStore: FSM восстанавливается из снапшота,
...
func TestRestoreFromSnapshotStore_RestoresFSMAndIndex(t *testing.T) {
	...
	// После полного успокоения цикла снапшотов фиксируем финальный индекс:
	// при рестарте restore берёт НОВЕЙШИЙ снапшот из стора.
	cm1.mu.Lock()
	...
	// кластере продвижение происходит при коммите новой команды —
...
// нарушений инвариантов: пустой store при усечённом логе, снапшот старее
...
func TestRestoreFromSnapshotStore_FailFastGap(t *testing.T) {
	...
	// 2. Снапшот в сторе старее lastSnapshotIndex из storage → ошибка.
	...
	// 3. Дыра между снапшотом и логом → ошибка.
	cm = &ConsensusModule{}
...
// самовосстановления (ревью HIGH-2): снапшот в durable-сторе новее
...
func TestRestoreFromSnapshotStore_SelfHealsStalePersistKey(t *testing.T) {
	...
	// Финальный индекс снапшота после успокоения цикла снапшотов.
	cm1.mu.Lock()
	snapIdx := cm1.cmState.lastSnapshotIndex
	cm1.mu.Unlock()
	...
	// Имитация крэша между Close снапшота и persistToStorage:
...
func TestRestoreFromSnapshotStore_EmptyLogWithSnapshot(t *testing.T) {
	...
	// Снапшот с индексом 5 в durable-сторе.
...
// снапшот-ключей для компактированного лога (TASK-008).
func TestCheckSnapshotKeysConsistency(t *testing.T) {
	// Компактированный лог без снапшот-ключей → ошибка.
	...
	// Полный лог (Index от 0) без снапшот-ключей → старт разрешён.
	cm = &ConsensusModule{storage: storage}
...
// TestRestoreFromSnapshotStore_RestoresConfiguration проверяет (TASK-010),
...
func TestRestoreFromSnapshotStore_RestoresConfiguration(t *testing.T) {
	...
	// Снапшот с ConfigIndex = 7 новее конфигурации лога (latestIndex = 3) —
	// после restore используется конфигурация снапшота.
	...
	// Снапшот с ConfigIndex = 2 старее конфигурации лога (latestIndex = 3) —
