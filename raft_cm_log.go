...
// logPosition возвращает позицию в срезе cm.cmState.log для записи с данным Index.
...
// несовпадении термов на follower или сжатии.
// Требует удержания cm.mu (Lock).
...
func (cm *ConsensusModule) dispatchLogs(applyLogs []*logFuture) {
	...
	if newCommitIndex := cm.leaderState.commitmentTracker.getCommitIndex(); newCommitIndex > cm.cmState.commitIndex {
