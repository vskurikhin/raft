...
// по таймауту electionTimeout() >= DefaultReelectionTimeout = 430 мс, всё это
// время флаг leadershipTransferInProgress удерживается — запас относительно
// _pollInterval не ниже 43x.
//
...
// чем через DefaultReelectionTimeout - DefaultHeartbeatTimeout = 430 - 33 = 397 мс от
...
// окно гарантированно живёт не меньше 397 мс при любом розыгрыше таймеров,
// а флаг наблюдается не позднее чем через _pollInterval = 10 мс после
// установки — запас до гарантированного закрытия окна не меньше 387 мс.
//
