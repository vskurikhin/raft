...
// NewHarness создаёт новый тестовый Harness, инициализированный
// n серверами, соединёнными друг с другом.
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			if i != j {
				...
				_, _ = fmt.Fprintf(os.Stderr, "connected to peer %d:%d peer listen address: %s\n", i, j, ns[j].GetListenAddr())
			}
		}
		connected[i] = true
	}
